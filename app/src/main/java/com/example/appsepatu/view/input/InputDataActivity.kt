package com.example.appsepatu.view.input
...
import com.example.appsepatu.viewmodel.InputDataViewModel
...
class InputDataActivity : AppCompatActivity() {
    ...
    private fun setInitLayout() {
        strKategori = resources.getStringArray(R.array.kategori_sepatu)
        strHarga = resources.getStringArray(R.array.harga_perjumlah)
        ...
        spKategori.setOnItemSelectedListener(object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(
                ...
                if (inputBerat.getText().toString() != "") {
                    countJumlah = inputBerat.getText().toString().toInt()
                    setTotalPrice(countJumlah)
                } else {
        ...
        inputBerat.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(editable: Editable) {
                ...
                if (editable.length > 0) {
                    countJumlah = editable.toString().toInt()
                    setTotalPrice(countJumlah)
                } else {
                    inputHarga.setText(rupiahFormat(countHarga))
                }
        ...
        inputTanggal.setOnClickListener { view: View? ->
            val tanggalBeli = Calendar.getInstance()
            val date =
                OnDateSetListener { view1: DatePicker?, year: Int, monthOfYear: Int, dayOfMonth: Int ->
                    tanggalBeli[Calendar.YEAR] = year
                    tanggalBeli[Calendar.MONTH] = monthOfYear
                    tanggalBeli[Calendar.DAY_OF_MONTH] = dayOfMonth
                    val strFormatDefault = "d MMMM yyyy"
                    val simpleDateFormat = SimpleDateFormat(strFormatDefault, Locale.getDefault())
                    inputTanggal.setText(simpleDateFormat.format(tanggalBeli.time))
                }
            DatePickerDialog(
                this@InputDataActivity, date,
                tanggalBeli[Calendar.YEAR],
                tanggalBeli[Calendar.MONTH],
                tanggalBeli[Calendar.DAY_OF_MONTH]
            ).show()
        }
    }

    private fun setTotalPrice(jumlah: Int) {
        countTotal = countHarga * jumlah
    ...
    private fun setInputData() {
        btnCheckout.setOnClickListener { v: View? ->
            ...
            if (strNama.isEmpty() or strTanggal.isEmpty() or strAlamat.isEmpty() or (strKategori.size == 0) or (countJumlah == 0) or (countHarga == 0)) {
                Toast.makeText(
